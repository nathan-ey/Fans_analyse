...
class User extends IndexBase
{
    ...
    function get_type_admins()
    {

        try {
            return DbManager::getInstance()->invoke(function ($client) {
                $res = AdminTypeModel::invoke($client)->all(['level' => 0]);
    ...
    function add_admins()
    {
        ...
        try {
            ...
            if ($Method_type == "GET") {
                ...
                if (!isset($data['limit']) || !isset($data['page']) || !isset($data['level']) || !isset($data['status'])) {
                ...
                if ($data['level'] == 0) {  #获取管理员
                    $list = $model->field("user.id,user.user,user.password,admin_type.name,user.created_at,user.updated_at,user.status,user.remark")
                        ->join("admin_type", 'admin_type.id=user.grade')->all(['admin_type.level' => 0, 'status' => $data['status']]);
                } else {
                    #获取普通用户
                    $list = $model->field("user.id,user.user,user.password,admin_type.name,user.created_at,user.updated_at,user.status,user.remark")
                        ->join("admin_type", 'admin_type.id=user.grade')->where('(admin_type.level>0)')->all(['status' => $data['status']]);
                }
